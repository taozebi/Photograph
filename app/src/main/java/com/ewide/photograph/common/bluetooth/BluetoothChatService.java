...
public class BluetoothChatService {
    ...
    public BlueToothCallBackListener bluelistener;
     ...
     * @param out 
     */
    public void write(byte[] out,BlueToothCallBackListener listener) {
...
//                    String strr = bytes2HexString(buffer);
                    System.out.println("获取到的消息：readMessagereadStr:"+readStr);
                    System.out.println("获取到的消息：readMessagestr:"+str);
//                    System.out.println("获取到的消息：readMessagestrr:"+strr);
                    if(bytes>0)
                    {
                    	if (str.endsWith("0D")) {
                            ...
                            .sendToTarget();
                    		System.out.println("获取到的消息：readMessage2:"+buffer1.length+":::::"+buffer1);
                    		str1 = "";
						}
                    	else{
                    		if (!str.contains("0A")) {
...
//		                            .sendToTarget();
//									System.out.println("获取到的消息：readMessage3:"+bytes+":::::"+buffer);
									byte[] buffer1 = (str1+readStr).getBytes();
		                    		mHandler.obtainMessage(MESSAGE_READ, buffer1.length, -1, buffer1)
		                            .sendToTarget();
		                    		System.out.println("获取到的消息：readMessage3:"+buffer1.length+":::::"+buffer1);
		                    		str1 = "";
                                    //回调接口
                                    if (bluelistener != null){
//                                    // 读取到的数据
                                        String readMessage = new String(buffer1, 0, buffer1.length);
                                        bluelistener.onFinish(readMessage);
                                    }
								}
