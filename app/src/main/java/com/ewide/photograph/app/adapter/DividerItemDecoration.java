...
public class DividerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public DividerItemDecoration(Context context, List<Map<String,Object>> mDatas) {
        mContext = context;
        this.mDatas = mDatas;
        // 轴点画笔(红色)
        mPaint = new Paint();
        ...
        // 此处设置了两只分别设置 时分 & 年月
        mPaint1 = new Paint();
        mPaint1.setColor(mContext.getResources().getColor(R.color.contentTextColorBlue));
        mPaint1.setTextSize(50);

        mPaint2 = new Paint();
        mPaint2.setColor(mContext.getResources().getColor(R.color.contentTextColor));
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
             ...
             */
            // 轴点 = 圆 = 圆心(x,y)
            float centerx = child.getLeft()- itemView_leftinterval / 6;
            ...
            // 设置文本起始坐标
            float Text_x = child.getLeft() - itemView_leftinterval * 11 / 12;
            float Text_y = upLine_bottom_y;
            Map<String,Object> map = mDatas.get(index);
            // 设置日期绘制位置
            String datetime = (String) map.get("nodetime");
            if(!TextUtils.isEmpty(datetime)&&datetime.contains(" ")){
                String[] times = datetime.split(" ");
                if(times.length>0){
                    c.drawText(times[1], Text_x, Text_y, mPaint1);
                    c.drawText(times[0], Text_x, Text_y + 50, mPaint2);
                }
            }
            ...
            /*switch (index) {
                case (0):
                    // 设置日期绘制位置
                    c.drawText("13:40:20", Text_x+10, Text_y, mPaint1);
                    c.drawText("2017-04-03", Text_x + 5, Text_y + 50, mPaint2);
                    break;
                case (1):
                    // 设置日期绘制位置
                    c.drawText("17:33:20", Text_x+10, Text_y, mPaint1);
                    c.drawText("2017-04-03", Text_x + 5, Text_y + 50, mPaint2);
                    break;
                case (2):
                    // 设置日期绘制位置
                    c.drawText("20:13:20", Text_x+10, Text_y, mPaint1);
                    c.drawText("2017-04-03", Text_x + 5, Text_y + 50, mPaint2);
                    break;
                case (3):
                    // 设置日期绘制位置
                    c.drawText("11:40:20", Text_x+20, Text_y, mPaint1);
                    c.drawText("2017-04-04", Text_x + 5, Text_y + 50, mPaint2);
                    break;
                case (4):
                    // 设置日期绘制位置
                    c.drawText("13:20:20", Text_x+10, Text_y, mPaint1);
                    c.drawText("2017-04-04", Text_x + 5, Text_y + 50, mPaint2);
                    break;
                case (5):
                    // 设置日期绘制位置
                    c.drawText("22:40:20", Text_x+10, Text_y, mPaint1);
                    c.drawText("2017-04-04", Text_x + 5, Text_y + 50, mPaint2);
                    break;
                default:
